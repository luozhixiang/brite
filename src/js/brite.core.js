...
/**
 * @namespace brite.ui is used to managed the lifescycle of UI components (create, display, and destroy)
 * 
 */
brite.ui = {};
(function($){
	 ...
	 *                              Note 1) If ctx.parent is absent from the component definition and from this method call, the brite.ui will not append the returned element to the DOM.
	 ...
	 * 								   component.destroy()                  (optional) This will get called when $.bRemove or $.bEmpty is called on a parent (or on the element for $.bRemove). It will get called before this component htmlElement will get removed<br />
	 * 								   component.postDestroy()              (optional) This will get called when $.bRemove or $.bEmpty is called on a parent (or on the element for $.bRemove). It will get called after this component htmlElement will get removed<br />
	 * 								
	 */
	brite.ui.registerComponent = function(name,config,componentFactory){
		var def = {};
		def.name = name;
	 ...
	 * @param {string} name
	 */
	brite.ui.instantiateComponent = function(name){
	...
	brite.ui.registerTransition = function(name,transition){
		_transitions[name] = transition;
	}

	brite.ui.getTransition = function(name){
	 ...
	 * @param {Object} config (optional) config override the component's config (see {@link brite.ui.registerComponent} config params for description)
	 * @return {Component} return the component instance.
	 */
	brite.ui.display = function(componentName,data,config){
		return process(componentName,data,config);
	};
	
	/**
	 * Same as brite.ui.display but bypass the build() step (postDisplay() will still be called). 
	 * So, this will create a new component and attach it to the $element and call postDisplay on it.
	 * 
	 */
	brite.ui.attach = function(componentName,$element,data,config){
	...
	/**
	 * Config for the brite.ui module. 
	 * <ul>
	...
	brite.ui.defaultComponentConfig = {
		emptyParent: false,
		postDisplayDelay: 0
	}
	// ------ /Public Properties: Config ------ //	
	 ...
	 * 
	 *    <li>As of now, the component is loaded by loading (via sync-AJAX class) and adding the "components/[name].html" content to the "body" (can be overriden with brite.ui.config.componentsHTMLHolder)</li>
	 * 
	 ...
	 *   - the "component/[name].html" will call the briteui.registerComponent([name],componentDef) <br />
	 *   </li>
	 *  </ul>
	 * <br />
	...
	function loadComponent(name){
		...
		// if the component is not loaded, load it
			...
			//otherwise, do the loading 
				...
				$.ajax({
					url: brite.ui.config.componentsPath + name + ".html",
					async: true
				}).complete(function(jqXHR, textStatus){
					$(brite.ui.config.componentsHTMLHolder).append(jqXHR.responseText);
				});
			}
		}
	...
	function process(name,data,config,$element){
		...
		loaderDeferred.done(function(componentDef){
			...
			// TODO: an optimization point would be to add a "bComponentUnique" in the class for data-bcomponent that have a confi.unique = true
			//      This way, the query below could be ".bComponentUnique [....]" and should speedup the search significantly on UserAgents that supports the getElementsByClassName
			if (config.unique){
	...
	function renderComponent (component,data,config){
		...
		if (config.transition){
			var transition = brite.ui.getTransition(config.transition);
			
			if (transition) {
				transition(component,data,config);
			}else{
				brite.log.error("Transition [" + config.transition + "] not found. Transitions need to be registered via brite.ui.registerTranstion(..) before call.");
			}
		}
	...
	function instantiateComponent(componentDef){
		...
		if (componentFactory ){
			...
			// if it is a plainObject, then, we clone it (NOTE: We do a one level clone)
				...
				brite.log.error("Invalid ComponentFactory for component [" + componentDef.componentName + "]. Only types Function or Object are supported as componentFactory. Empty component will be created.");				
			}
		}else{
...
(function($) {
   ...
   * containing the brite.ui component for this name. 
   * 
   ...
   * For example: 
   * @example
   * var myComponent = $(thisDiv).bComponent("myComponent");
   * 
  ...
  $.fn.bComponent = function(componentName) {
    ...
    */
   $.fn.bComponentChildren = function(componentName){
   ...
   /**
    * Safely empty a HTMLElement of its children HTMLElement and bComponent by calling the preRemove and postRemove on every child components.
    * 
   ...
   $.fn.bEmpty = function(){
   	   return this.each(function(){
   	   	  ...
   	   	  var componentChildren = $this.bComponentChildren();
   ...
   /**
    * Safely remove a HTMLElement and the related bComponent by calling the preRemote and postRemove on every child components as
   ...
   function processDestroy(component){
   	// The if(component) is a safeguard in case destroy gets call twice (issue when clicking fast on test_brite.ui-02-transition....)
   	if (component){
 ...
 * 
 * @example
 *    brite.util.uuid(); // returns "92329D39-6F5C-4520-ABFC-AAB64544E172"
 *    brite.util.uuid(15);    // 15 character ID (default base=62), returns "VcydxgltxrVZSTV"
 *    brite.util.uuid(8, 2);  // returns "01001010"
 *    
...
brite.util.array = {
	 ...
	 * var vehiculeById = brite.util.array.toMap(myVehicules,"id");
...
(function($){
	 ...
	 */
	brite.ua.hasEvent = function(eventName){
	 ...
	 * <br /> <br />
	 * TODO: we might want to have a brite.ua.cssPrefix() since it will be the same prefix for other css properties as well
	 */
...
(function($){
	 ...
	 * @returns {Number}
	 */
	brite.gtx.prototype.scaleX = function(x,dontAddOffset){
		if (this.scallable){
			x *= this._ratio;
			if (!dontAddOffset) {
				x += this._xOffset;
	 ...
	 * @returns {Number}
	 */	
	brite.gtx.prototype.scaleY = function(y,dontAddOffset){
		if (this.scallable){
			y *= this._ratio;
			if (!dontAddOffset) {
				y += this._yOffset;
	 ...
	 * @returns {Number}
	 */
	brite.gtx.prototype.scaleVal = function(val){
		if (this.scallable){
