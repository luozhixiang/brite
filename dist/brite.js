...
(function($){
	 ...
	 *                              Note 1) If ctx.parent is absent from the component definition and from this method call, the brite will not append the returned element to the DOM.
	 ...
	 * @param {Object} config (optional) config override the component's config (see {@link brite.registerComponent} config params for description)
	...
	/**
	 * Same as brite.display but bypass the build() step (postDisplay() will still be called). 
	 ...
	 * 
	 *    <li>As of now, the component is loaded by loading (via sync-AJAX class) and adding the "components/[name].html" content to the "body" (can be overriden with brite.config.componentsHTMLHolder)</li>
	 * 
...
(function($) {
   ...
   * containing the brite component for this name. 
   * 
  ...
  $.fn.bComponent = function(componentName) {
   ...
   function processDestroy(component){
   	// The if(component) is a safeguard in case destroy gets call twice (issue when clicking fast on test_brite-02-transition....)
   	if (component){
...
(function($) {
	...
	/**
	 * Wrap the brite.dm.get(objectType,id) with a deferred result
	 */
	...
	/**
	 * Wrap the brite.dm.list(objectType,opts) with a deferred result
	 */
	...
	/**
	 * Wrap the brite.dm.create(objectType,data) with a deferred result
	 */
	...
	/**
	 * Wrap the brite.dm.update(objectType,id,data) with a deferred result
	 */
	...
	/**
	 * Wrap the brite.dm.remove(objectType,id) with a deferred result
	 */
